  ...
  if (isset($_POST["edit"])){ 
    ...
    $ConfirmPass = mysqli_real_escape_string($conn, $_POST["ConfirmPass"]);
      ...
      $hash_pass = password_hash($ConfirmPass, PASSWORD_DEFAULT); 
            ...
            $upquery="UPDATE users SET Full_Name = '$Full_Name', email = '$email', UserType = '$UserType' ,phone_Number='$phone_Number', Address='$Address',DateOfBirth='$DateOfBirth', User_Name='$User_Name',password = '$hash_pass' WHERE userId ='$userId';";
    ...
    if($conn->query($upquery) === TRUE){
      echo "<script type='text/javascript'>alert(\"User Details has been Updated.\");window.location.href='../dashboard.php';</script>";
       ...
       echo "<script type='text/javascript'>alert(\"User Details have not been Updated!\");window.location.href='../dashboard.php';</script>";
       exit();
